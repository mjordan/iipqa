...
/**
 * Checks for input for the Islandora Newspaper Batch module.
 */
class Newspapers extends ContentModelQaFramework
{
    ...
    {
        $this->issuePathsToTest = $this->reader->read($this->inputDirectory);

        $files_present = array();
        $this->numIssuePathsToTest = count($this->issuePathsToTest);
        $current_path_num = 0;
     ...
     * a metadata file and a thumnbail file in each newspaper issue directory.
     *
    ...
    public function checkBookFiles()
    {
        ...
        $this->issuePathsToTest = $this->reader->read($this->inputDirectory, true);
        $this->numIssuePathsToTest = count($this->issuePathsToTest);
        $current_path_num = 0;
        foreach ($this->issuePathsToTest as $issue_path) {
            ...
            if (!preg_match('#\.{1,2}$#', $issue_path)) {
                $this->progressBar->progressBar(
                    'Check for expected newspaper issue metadata files, page subdirectories, and page files',
                    $this->numIssuePathsToTest,
                    $current_path_num
                );
                if (is_dir($issue_path)) {
                    // Get all subdirs, then loop through them and
                    // check if they are numeric.
                    $page_dirs = $this->reader->read($issue_path);
                        ...
                        if (is_dir($page_dir)) {
                            ...
                            if (!in_array($filename, $allowed_metadata_files) &&
                                ...
                                $this->log->addWarning("File $filename is not an allowed " .
                                    "filename within newspaper issue directory $issue_path.");
                                $failures[] = $page_dir;
                            }
                        }
                    }
        ...
        if (count($failures)) {
            ...
            $this->log->addInfo("'Check for expected newspaper issue metadata files, page subdirectories, " .
                "and page files' test did not find any problems.");
            return true;
        }
    }
